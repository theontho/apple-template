...
@main
struct AppleTemplateCLI: ParsableCommand {
  static let configuration = CommandConfiguration(
    commandName: "apple-template",
    abstract: "Best-practice Swift CLI template for Apple platforms.",
    version: "0.1.0",
    subcommands: [Precheck.self, ConfigCommand.self, Run.self],
...
struct Run: ParsableCommand {
  ...
  mutating func run() throws {
    ...
    CLIOutput.log(
      "Starting apple-template", level: .info, configuredLevel: logLevel, quiet: options.quiet)
  ...
  static func greeting(name: String) -> String {
    "Hello, \(name) from apple-template!"
  }
}
